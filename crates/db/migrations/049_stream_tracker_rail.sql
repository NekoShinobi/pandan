-- Task.Progress is replaced by the account's Twitch/Kick utility-rail tracker.
-- Remove placed progress widgets, then rebuild the kind check without that legacy kind.
DELETE FROM dashboard_widgets WHERE kind = 'task-progress';
...
-- The placement marker distinguishes this system tracker from movable Live Channels widgets.
INSERT INTO dashboard_widgets
    (id, user_id, kind, workspace, position, size, config_json,
     grid_x, grid_y, grid_w, grid_h, created_at, updated_at)
SELECT users.id || '-stream-tracker', users.id, 'streams', 0,
       COALESCE((
           SELECT MAX(existing.position) + 1
           FROM dashboard_widgets existing
           WHERE existing.user_id = users.id AND existing.workspace = 0
       ), 0),
       'standard',
       '{"placement":"utility_rail","twitch_channels":[],"kick_channels":[]}',
       0, 0, 4, 4, datetime('now'), datetime('now')
FROM users
WHERE NOT EXISTS (
    SELECT 1
    FROM dashboard_widgets existing
    WHERE existing.user_id = users.id
      AND existing.kind = 'streams'
      AND json_extract(existing.config_json, '$.placement') = 'utility_rail'
);
