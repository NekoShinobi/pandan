...
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Announcements**  | Instance-wide administrator notes with sanitized Markdown, authenticated images, and account-scoped emoji reactions. Dashboard remains the initial page.                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| **YouTube**        | Channel subscriptions organized by category, a searchable Edit Sources menu for channel management and category ordering, a private Watch Later queue, mobile-friendly thumbnail or compact layouts, video actions in a three-dot menu, manual refresh, and shared metadata caching. |
...
| **Podcasts**       | An administrator-curated show catalogue for the whole instance, member requests with approve/reject review, per-account subscriptions, optional new-episode delivery to a chosen account-owned ntfy topic, a play queue, saved episodes, expandable show notes on every episode, and episodes downloaded once and then streamed from this server. Ready episodes can also be downloaded to the listener's device. Playback follows you across sections with skip-to-episode, rewind and forward, volume, and speed controls, and resumes where you left off. Volume starts at 80% and can be pushed to 200% for a quietly mastered show. Administrators can queue a show's whole back catalogue in one action.   |
...
| **Coding**         | Releases from GitHub, GitLab, Codeberg, Gitea, and Forgejo; connected accounts can also show owned repositories, open pull requests, and GitLab pipelines. A server worker refreshes the latest account-scoped snapshot hourly, while the page reads that cache unless a manual refresh is requested. Repositories can be organized and filtered with account-owned categories.                                                                                                                                                                                            |
...
| **Trading**        | An account-private watchlist that renders the last cached prices immediately. Yahoo Finance refreshes when the page opens or on demand; an optional encrypted Finnhub API key enables 20-second updates only while Trading remains open. Cached rows stay visible during refreshes, partial provider failures, and live-feed reconnects.                                                                                                                                                                                                                                            |
...
- YouTube channel metadata refreshes every two hours. Pandan tries configured Invidious first and YouTube's public uploads feed second. Recent-looking Invidious dates are checked against individual video metadata so older promoted uploads do not appear newly published. Shared channel portraits are stored in SQLite and refreshed at most every 24 hours; invalid or failed portrait responses are not cached.
