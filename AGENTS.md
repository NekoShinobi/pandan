...
- `ui/src/lib/JournalPage.svelte` — private recursive Markdown documents, file ordering, and per-document emoji logos.
...
- YouTube and RSS expose a page-level Refresh control in browsers and installed PWAs. It reloads
  the latest account-scoped server snapshot in place, retains filters and playback, and keeps
  existing records visible on failure. Upstream source refresh remains in Edit Sources.
...
- Calendar month grids default to Sunday-first and share the account's adjustable week-start
  preference. Every date in the Calendar product renders all visible items inside a bounded,
  independently scrollable day cell. Each subscribed calendar persists whether its month entries
  use full title listings or color dots, with full listings as the compatibility default. A
  subscribed calendar's listing name, color, source URL, and display mode are owner-editable; the
  custom name survives refresh, while changing the URL validates and fetches the replacement before
  atomically swapping the source and cached events. The dashboard right-rail calendar supports month
...
- YouTube filters uploads by category only. Channel subscriptions, category membership, and category
  ...
  Each video's channel avatar spans the channel-name and compact view-count/publication-date rows.
  Duration appears in an always-visible black badge at the thumbnail's bottom right. Both metadata fields are optional Invidious values;
  missing values, including Atom fallback refreshes, must preserve previously cached metadata.
...
- Coding provider data is refreshed into an account-scoped in-memory cache by a server worker every hour. Opening the page reads only the last completed cache snapshot, while the page's Refresh control forces a provider refresh. Project or credential mutations invalidate the snapshot and queue a replacement in the background. Preserve the generation check so an in-flight request cannot restore stale data after invalidation.
- Trading watchlists, provider settings, and quote snapshots are account-scoped. Opening Trading must return the SQLite cache before starting provider work. Without a credential, refresh Yahoo Finance once on page entry and on explicit request. A Finnhub key is optional, encrypted with `PANDAN_SECRET_KEY`, never returned to the browser, and drives the authenticated 20-second SSE refresh loop only while that page connection exists. Do not add a background market-data worker. A failed or partial refresh records a safe status message and preserves every last successful quote that the provider omitted.
...
- Browser notifications are an account-scoped, browser-local opt-in in the Bell panel. Request OS permission only from that control's user gesture. Announce new deliveries through the existing shell stream and recovery path, never the initial inbox snapshot or older-page loads. Prefer the active service worker's notification API, deduplicate across tabs using account-qualified IDs, and validate the account on notification clicks. Keep inbox/toast delivery working when permission or OS delivery fails. This is not Web Push: a running page supplies events, and the worker must still never cache notification records or API responses.
...
- Ollama is one administrator-managed instance integration and is disabled by default. Only administrators may configure it or start inference. Wall tagging is ad hoc only: send the server-generated thumbnail, return exactly the configured number of reviewable suggestions, and never attach them until the administrator saves the wall details. Do not add a background inference worker or expose filesystem paths, database credentials, or unrestricted image bytes to the model.
...
- New-episode ntfy notifications are private per-subscription routes. A listener may select only one of their own configured ntfy topics, and newly indexed episodes enqueue delivery in the same database transaction. Publishing stays server-owned, uses the encrypted account credential through the `notifications` network policy, and retries from a leased queue; the browser never publishes to ntfy directly.
- The Podcasts Refresh control actively refetches every feed the caller subscribes to through the existing network policy and refresh lease, with a short repeat-request cooldown. It must never regress to reloading only the cached overview.
...
- The instance logo and favicon are administrator-managed assets stored in SQLite and publicly
  readable for signed-out chrome. Accept SVG uploads only by rendering them without external
  resources and storing the result as PNG; never serve administrator-supplied SVG markup from
  Pandan's origin. Bound logo uploads to 10 MB and favicon uploads to 1 MB, and retain the packaged
  Pandan fallbacks when either asset is absent.
...
- Server-side remote destinations default to public HTTPS. Administrator-managed `network_access_rules` may allow or deny an exact scheme, host, and port for all integrations or one of `rss`, `calendar`, `contacts`, `podcasts`, `notifications`, `coding`, `images`, `youtube`, `widgets`, `jellyfin`, or `ai`. A deny match always wins; only an explicit allow may authorize HTTP or a private/reserved destination. Keep rules administrator-only and capped at 128.
...
- Every successful RSS refresh advances `rss_subscriptions.refresh_generation` and stamps returned
  entries with that generation. Inbox may retain older generations, Current shows only each source's
  configured 1–200 latest entries from the latest successful generation (default 25), and a failed
  refresh must never advance or blank that snapshot. Current entries remain retention-exempt while
  the source still exposes them. Dashboard RSS widgets select
